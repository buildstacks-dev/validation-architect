...
7. **Seeded-mode controls stay sweepable.** Write each negative-control test so its seeded violation activates when the `VA_SEEDED_CONTROL` environment variable names its control id: unseeded, the detector run is green; seeded, it goes red. The repo's scheduled control-sweep template (VA-ENF-008, `enablement/sweep/control-sweep.mjs` where adopted) re-proves this on a cadence for every landed control — a control that cannot go red fails the sweep, and that failure is a defect against the harness, not noise.
8. **Manual findings enter through the schema.** When the ticket originates from a reported bug or exploratory finding (a GitHub issue, an operator report), the intake path is: record the finding in `model/sources.yaml` with `kind: finding` — `locator` names the issue URL/id, `path`/`quote` names the exact triggering input — derive the family it deposits (cite the finding source in the family's `source_ids`), and set `finding_ref` on the fix ticket. The compiler refuses a `finding` source without its triggering input and a `finding_ref` ticket that owns no family, so a fix cannot exist without its claim. The detector lands in the same change as the fix (rule 6).
