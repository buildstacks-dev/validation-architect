...
18. **Rollups are derived, never authored.** Every count, summary line, and status total in an artifact or report is computed from the rows it summarizes — never typed alongside them. A hand-written total drifts from its own table on the first edit, and a reader who trusts the summary then sees a healthier picture than the evidence supports. This is "no green by absence" applied to the harness's own reporting, and it holds for the audit's findings counts and the case catalog's closure counts alike.
...
**The module map is a commitment, not a recommendation.** Each entry carries a status — `deep-pass-done` (naming the module policy's path), `deep-pass-pending` (with the reason it is queued rather than done), or `deliberately-shallow` (with the reason, exactly like a declared-empty lane). It lands in `validation-policy.yaml` under `module_map:`, and it constrains what the product-scope campaign may claim: while any module is `deep-pass-pending`, closure is reported as **complete-at-product-granularity**, never as complete. A module map whose entries carry no obligation is the easiest way for a large product to end up with one flat catalog covering everything shallowly and nothing deeply — which reads as closure and is not.
...
**Interaction mode is detected, never asked.** If no ratified `validation-policy.yaml` exists for this scope, the run is **onboarding**: teach-first, every hard stop, the full concept loop. If one does exist, the run is **steady-state** and the stance inverts — do not re-teach a concept the owner has already ratified against, do not re-elicit a settled decision, and do not re-present the artifact set for confirmation. Open with what *changed* and what that implies. An owner on their fifth run being walked through the boundary concept again will stop reading, and a skill that costs a full ceremony per run gets run once.

**Run state is separate from the design contract.** `validation-policy.yaml` is human-ratified and tighten-only; it says what must be true. A re-run also needs to know what was already derived and against what — that is machine-written, freely rewritten, and belongs in its own file (`harness-state.yaml`): the source revision each derivation ran against, per-artifact staleness, the substrate inventories with their digests, ratification records with dates, and the module-map statuses. Keeping them in one file means a machine write lands on a ratified artifact, which is exactly the confusion the tighten-only rule exists to prevent.
...
**What a steady-state run may do unattended:** re-derive substrate inventories, add cases against existing structure, record drift, regenerate rollups and the owner-facing documents, and re-check that enumerating guards still cover their class. **What still stops for a human:** a new invariant, a new boundary, a tier change, a newly opened product-truth finding, a rubric change, and any loosening of anything. A ratified decision stays ratified across runs and is never re-asked; the state file records what was ratified and when.
