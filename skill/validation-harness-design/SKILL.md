...
3. **Production incidents** — every bug becomes a regression test; every bad LLM output in prod becomes a golden case. Non-negotiable. The intake path is schema, not convention: record the manual finding as a `sources.yaml` entry with `kind: finding` (its `locator` names the issue, its `path`/`quote` names the exact triggering input), give the fix ticket a `finding_ref` citing it — the compiler refuses a fix ticket that owns no family, so the claim exists before the fix merges — and land the detector in the same change as the fix.
