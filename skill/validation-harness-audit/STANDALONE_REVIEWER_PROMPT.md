...
16. When `validation-design/model/` exists, compile all eight logical YAML files before using any generated view. The compiler-clean model identity is the authoritative claims baseline; test inventory and exact-revision evidence remain separate. Keep namespaced IDs, reject stale views, and record contradictions as findings. A lane declared empty with a reason is a decision; silent absence is a finding. Explicit coexistence protections remain read-only for the reviewer.
...
2. A compiler-clean `validation-design/model/*.yaml` graph when present — keep its namespaced product-structure IDs as authoritative claim IDs; treat generated catalog/backlog/owner/trace Markdown as projections, not another source.
...
When a checked model exists, additionally audit compiler and lane conformance: verify its report and generated views share one identity; join the separately observed test inventory to compiled families in both directions; map evidence onto the six validation layers; verify required gates really block; verify inherited obligations only tighten; verify every family has honest status, ownership, provenance, layer/oracle rationale, a red-capable negative control, and a planned test or evidence declaration appropriate to its lane; verify deterministic live/eval defects deposited L1/L2 detectors; verify golden-set, boundary-double, adapter, matrix-closure, harness-self-test, agent-routing, and coexistence disciplines. For active `L-ACC`, verify human rubric ratification, guardrail placement, realistic stable inputs, sealed answers, per-axis independence, self-report separation, campaign-invariant separation, durable staged gates, fresh authorization, an explicit release relationship, and incomplete/inconclusive outcomes for missing calibration or work.
