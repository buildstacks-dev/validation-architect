...
let claudeAgentSdk: Promise<ClaudeAgentSdk> | undefined;
let codexSdk: Promise<CodexSdk> | undefined;

async function loadClaudeAgentSdk(): Promise<ClaudeAgentSdk> {
  try {
    return await (claudeAgentSdk ??= import("@anthropic-ai/claude-agent-sdk"));
  } catch {
    claudeAgentSdk = undefined;
    throw new Error(
      `Missing optional peer @anthropic-ai/claude-agent-sdk. Install exactly @anthropic-ai/claude-agent-sdk@${CLAUDE_AGENT_SDK_VERSION} before running this design command.`,
    );
  }
}
...
async function loadCodexSdk(): Promise<CodexSdk> {
  try {
    return await (codexSdk ??= import("@openai/codex-sdk"));
  } catch {
    codexSdk = undefined;
    throw new Error(
      `Missing optional peer @openai/codex-sdk. Install exactly @openai/codex-sdk@${CODEX_SDK_VERSION} before running this design command.`,
    );
  }
}
