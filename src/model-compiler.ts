...
export function compileValidationModel(
  ...
  const tickets = mapRows<BacklogTicket>(rows(parsed, files, diagnostics, "backlog.yaml", "tickets"), diagnostics, files, "backlog.yaml", (row, id) => {
    ...
    return { id, title, wave, status: status as BacklogTicket["status"], owner, executor, lane, layer, acceptance_criteria: acceptanceCriteria, family_ids: familyIds, ...(texts(row, "depends_on") ? { depends_on: texts(row, "depends_on") as string[] } : {}) };
  });
