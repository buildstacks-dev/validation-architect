...
export function compileValidationModel(
  ...
  const sources = mapRows<ModelSource>(rows(parsed, files, diagnostics, "sources.yaml", "sources"), diagnostics, files, "sources.yaml", (row, id) => {
    ...
    if (!kind || !["doc", "rambling", "simulated", "proposed", "finding"].includes(kind)) { shapeError(diagnostics, files, "sources.yaml", id, "kind", "Use doc, rambling, simulated, proposed, or finding."); return undefined; }
  ...
  const structures = mapRows<ProductStructure>(rows(parsed, files, diagnostics, "structures.yaml", "structures"), diagnostics, files, "structures.yaml", (row, id) => {
    rejectUnknownFields(diagnostics, files, "structures.yaml", id, row, ["id", "kind", "title", "meaning", "owner", "source_ids", "acceptance_criteria", "error_criteria", "failure_modes", "changed_paths", "criticality", "criticality_reason"]);
    ...
    return { id, kind, title, meaning, owner, source_ids: sourceIds, ...(texts(row, "acceptance_criteria") ? { acceptance_criteria: texts(row, "acceptance_criteria") as string[] } : {}), ...(texts(row, "error_criteria") ? { error_criteria: texts(row, "error_criteria") as string[] } : {}), ...(texts(row, "failure_modes") ? { failure_modes: texts(row, "failure_modes") as string[] } : {}), ...(texts(row, "changed_paths") ? { changed_paths: texts(row, "changed_paths") as string[] } : {}), ...(criticality && ["C0", "C1", "C2", "C3", "C4"].includes(criticality) ? { criticality } : {}), ...(text(row, "criticality_reason") ? { criticality_reason: text(row, "criticality_reason") as string } : {}) };
  });
  ...
  const lanes = mapRows<ValidationLane>(laneRows, diagnostics, files, "policy.yaml", (row, id) => {
    rejectUnknownFields(diagnostics, files, "policy.yaml", id, row, ["id", "title", "kind", "status", "requirement", "triggers", "command", "max_duration_seconds", "authorization", "reason"]);
  ...
  const families = mapRows<ValidationFamily>(rows(parsed, files, diagnostics, "families.yaml", "families"), diagnostics, files, "families.yaml", (row, id) => {
    rejectUnknownFields(diagnostics, files, "families.yaml", id, row, ["id", "title", "meaning", "structure_ids", "owner", "source_ids", "lane", "status", "layer", "oracle", "risk", "purpose", "control_ids", "covers_failure_modes", "ticket", "planned_tests", "evidence", "blocked_by", "reason", "exclusions"]);
    ...
    const purpose = text(row, "purpose") as ValidationFamily["purpose"] | undefined;
    if (purpose && !["behavior", "triangulation"].includes(purpose)) { shapeError(diagnostics, files, "families.yaml", id, "purpose", "Use purpose: behavior (default) or triangulation (failure-localization unit layer)."); return undefined; }
    ...
    return { id, title, meaning, structure_ids: structureIds, owner, source_ids: sourceIds, lane, status: status as ValidationFamily["status"], ...(ticket ? { ticket } : {}), ...(layer && ["L1", "L2", "L3", "L4", "L5", "L6"].includes(layer) ? { layer } : {}), ...(text(row, "oracle") ? { oracle: text(row, "oracle") as string } : {}), ...(text(row, "risk") ? { risk: text(row, "risk") as string } : {}), ...(purpose ? { purpose } : {}), ...(texts(row, "control_ids") ? { control_ids: texts(row, "control_ids") as string[] } : {}), ...(texts(row, "covers_failure_modes") ? { covers_failure_modes: texts(row, "covers_failure_modes") as string[] } : {}), ...(texts(row, "planned_tests") ? { planned_tests: texts(row, "planned_tests") as string[] } : {}), ...(evidenceState && evidencePath && ["complete", "incomplete", "inconclusive", "unobserved"].includes(evidenceState) ? { evidence: { state: evidenceState, path: evidencePath } } : {}), ...(text(row, "blocked_by") ? { blocked_by: text(row, "blocked_by") as string } : {}), ...(text(row, "reason") ? { reason: text(row, "reason") as string } : {}), ...(texts(row, "exclusions") ? { exclusions: texts(row, "exclusions") as string[] } : {}) };
  });
  const tickets = mapRows<BacklogTicket>(rows(parsed, files, diagnostics, "backlog.yaml", "tickets"), diagnostics, files, "backlog.yaml", (row, id) => {
    rejectUnknownFields(diagnostics, files, "backlog.yaml", id, row, ["id", "title", "wave", "status", "owner", "executor", "lane", "layer", "acceptance_criteria", "family_ids", "depends_on", "finding_ref"]);
    ...
    const findingRef = text(row, "finding_ref");
    if (findingRef && !familyIds?.length) { shapeError(diagnostics, files, "backlog.yaml", id, "family_ids", "A fix ticket cannot exist without a claim: a ticket carrying finding_ref must own at least one family whose detector lands with the fix."); return undefined; }
    ...
    return { id, title, wave, status: status as BacklogTicket["status"], owner, executor, lane, layer, acceptance_criteria: acceptanceCriteria, family_ids: familyIds, ...(dependsOn ? { depends_on: dependsOn } : {}), ...(findingRef ? { finding_ref: findingRef } : {}) };
  });
  ...
  validateModel(model, {
    ...
    codedError: (code, file, concept, message, correction) => diagnostics.push({ code, severity: "error", concept, location: scalarLocation(file, files[file] ?? "", concept), message, correction }),
  });
