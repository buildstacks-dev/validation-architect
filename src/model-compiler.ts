...
export function compileValidationModel(
  ...
  const families = mapRows<ValidationFamily>(rows(parsed, files, diagnostics, "families.yaml", "families"), diagnostics, files, "families.yaml", (row, id) => {
    rejectUnknownFields(diagnostics, files, "families.yaml", id, row, ["id", "title", "meaning", "structure_ids", "owner", "source_ids", "lane", "status", "layer", "oracle", "risk", "control_ids", "covers_failure_modes", "ticket", "planned_tests", "evidence", "blocked_by", "reason", "exclusions"]);
