...
export function buildRelationshipGraph(input: RelationshipGraphInput): RelationshipGraph {
  ...
  for (const family of model.families.filter((item) => item.status === "implementable")) {
    ...
    if (tests.length === 0 && actualEvidence.length === 0) finding(findings, { code: "IMPLEMENTATION_MISSING", level: "red", subject_id: family.id, message: `${family.id} has no observed test or evidence implementation.`, correction: "Implement the declared detector/evidence or change the reviewed family status." });
    if (actualEvidence.length === 0) finding(findings, { code: "EVIDENCE_ARTIFACT_MISSING", level: "red", subject_id: family.id, message: `${family.id} has no revision-bound evidence artifact.`, correction: "Collect and integrity-bind evidence for the exact graph identity." });
    if (family.evidence && !actualEvidence.some((item) => item.path === family.evidence?.path)) finding(findings, { code: "EVIDENCE_ARTIFACT_MISSING", level: "red", subject_id: family.id, message: `${family.id} declares ${family.evidence.path}, but matching evidence was not observed.`, correction: "Collect the declared artifact at the exact graph identity." });
    for (const planned of family.planned_tests ?? []) if (!tests.some((test) => test.path === planned)) finding(findings, { code: "PLANNED_IMPLEMENTATION_DRIFT", level: "red", subject_id: family.id, message: `${family.id} planned test ${planned} was not observed.`, correction: "Land the planned test or revise the reviewed model and recompile." });
    const landed = model.tickets.find((ticket) => ticket.id === family.ticket)?.status === "landed";
