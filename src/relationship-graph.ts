...
export function buildRelationshipGraph(input: RelationshipGraphInput): RelationshipGraph {
  ...
  for (const family of model.families.filter((item) => item.status === "implementable")) {
    ...
    const pendingControls = joined.unimplemented_control_ids.filter((id) => controls.get(id)?.family_id === family.id);
    if (pendingControls.length > 0 && nonLanded) finding(findings, { code: "CONTROL_IMPLEMENTATION_PENDING", level: "partial", subject_id: family.id, message: `${family.id} declares ${pendingControls.join(", ")} with no implementing test while its owner ticket ${family.ticket} is ${ticketStatus}.`, correction: "Land the declared negative-control test before marking the owner ticket landed or reporting product green." });
